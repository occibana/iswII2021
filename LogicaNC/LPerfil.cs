...
namespace LogicaNC
{
    public class LPerfil
    {
        ...
        public UPerfil cargardatos(URegistro datosSession)
        {
            ...
            URegistro datos = new DAOLogin(_context).mostrarDatos(datosSession);
            ...
            {
                var verificar = new DAOSeguridad(_context).verificarvencimientomembresia(perfil.Datos.Id);
                if (verificar != null)
                {
                    perfil.Datos.Id =datos.Id;
                    perfil.Datos.Idestado = 0;
                    new DAOSeguridad(_context).actualizarmembresia(perfil.Datos);
        ...
        public string cerrarsession(URegistro sessionId)
        {
            try
            {
                URegistro datos = new DAOLogin(_context).mostrarDatos(sessionId);
                new DAOSeguridad(_context).cerrarAcceso(datos.Id);
                new DAOSeguridad(_context).borrarTokenLogin(datos);
            ...
            if (foto != null)
            {
                ...
                if ((ext == ".jpg" || ext == ".png" || ext == ".jpeg"))//menor a 1MB en bytes  (tam < 1048576)
                {
                    ...
                    try
                    {
                        ...
                        nuevodat.Id = session.Id;
                        nuevodat.Fotoperfil = direccion;
                        new DAOLogin(_context).actualizarfoto(nuevodat);
