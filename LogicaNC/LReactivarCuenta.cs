...
namespace LogicaNC
{
    ...
    public class LReactivarCuenta
    {
        ...
        public LReactivarCuenta(Mapeo context)
        {
            _context = context;
        }
        ...
        public URegistro recuperarContrasena(string codigo, URegistro user)
        {
            URegistro datos = new URegistro();
            datos.Id = user.Id;
            datos.Contrasena = user.Contrasena;
            UToken token = new DAOSeguridad(_context).validartoken(codigo, datos);


            if (token == null)
            {
                datos.Mensaje = "Codigo no valido, verifique el codigo en su correo";
                //this.RegisterStartupScript("mensaje", "<script type='text/javascript'>alert('No puede acceder sin un link de recuperacion, verifique su correo');window.location=\"Login.aspx\"</script>");
            }
            else if (token.Fecha_caducidad < DateTime.Now)
            {
                datos.Mensaje = "Tiempo de validez de codigo ha terminado";
                //this.RegisterStartupScript("mensaje", "<script type='text/javascript'>alert('Tiempo de validez de token ha terminado');window.location=\"Login.aspx\"</script>");
            }
            else
            {
                datos = new LReactivarCuenta(_context).actualizarContrasena(datos);
            ...
            //else
            //{
            //    datos.Mensaje = "Codigo no valido o expirado";
            //    return datos;
            //}
            //return datos;
        }
        ...
        public URegistro actualizarContrasena(URegistro datosLogin)
        {
            ...
            try
            {
                ...
                {
                    new DAOSeguridad(_context).actualizarcontrasenarecuperacion(datosLogin);
